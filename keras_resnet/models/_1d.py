...
class ResNet1DRegression(keras.Model):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        if include_top:
            ...
            x = keras.layers.Dense(128, activation="relu", name="fc999")(x)
            x = keras.layers.Dense(preds, name="fc1001")(x)
            
            if "outputs" in kwargs.keys():
              x = kwargs["outputs"]
              kwargs.pop("outputs")
            
            super(ResNet1DRegression, self).__init__(inputs, x, *args, **kwargs)
        else:
            # Else output each stages features
            super(ResNet1DRegression, self).__init__(inputs, outputs, *args, **kwargs)
...
class ResNet1D18Regression(ResNet1DRegression):
    """
    ...
    Usage:
        ...
        >>> model.compile("adam", "categorical_crossentropy", ["accuracy"])
    """
    def __init__(self, inputs, blocks=None, include_top=True, preds=2, freeze_bn=False, *args, **kwargs):
        if blocks is None:
            blocks = [2, 2, 2, 2]

        super(ResNet1D18Regression, self).__init__(
            inputs,
            blocks,
            block=keras_resnet.blocks.basic_1d,
